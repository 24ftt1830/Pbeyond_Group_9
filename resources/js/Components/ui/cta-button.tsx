...
export default function CtaButton({ href, label }: CtaButtonProps) {
    ...
    const animate = () => {
        ...
        if (labelRef.current) {
            ...
            const lightness = Math.round(lerp(100, 100, progress));
    ...
    return (
        <Link
            ...
            className="relative inline-flex items-center gap-4 overflow-hidden border border-white/20 bg-white/10 backdrop-blur-sm px-5 py-3 cursor-pointer"
        >
            {/* Fill — white/25 so text remains legible on top */}
            <div
                ref={fillRef}
            ...
            <div
                ref={squareRef}
                className="relative z-10 w-5 h-5 bg-white shrink-0"
            ...
            <span
                ...
                style={{ transform: 'translateX(0px)', color: 'hsl(0, 0%, 100%)' }}
            >
                {label}
            </span>
        </Link>
