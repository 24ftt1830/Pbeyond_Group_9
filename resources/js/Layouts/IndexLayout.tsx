            ...
            <div className="relative bg-primary">
                ...
                >
                    <div className="mx-3 md:mx-4 mt-3 md:mt-4">
                        <div className="flex items-center justify-between px-4 md:px-6 py-3 rounded-2xl bg-white/15 backdrop-blur-xl border border-white/10 shadow-sm">
                            ...
                            <div className="flex items-center gap-3">
                                {auth.user ? (
                                    <Link
                                        ...
                                        className="text-sm px-3 md:px-4 py-1.5 uppercase bg-white hover:bg-white/90 transition-colors duration-150"
                                    >
                                        Dashboard
                                    </Link>
                                ) : (
                                    <Link
                                        href={route('login')}
                                        className="text-sm px-3 md:px-4 py-1.5 bg-white uppercase hover:bg-white/90 transition-colors duration-150"
                                    >
                                        Log in
                                    </Link>
                                )}
                ...
                <div className="fixed bottom-0 w-full" style={{ height: `${FOOTER_HEIGHT}px` }}>
                    <footer className="relative h-full w-full overflow-hidden">
                        ...
                        <div className="relative z-[99] flex h-full flex-col justify-between text-white px-6 md:pl-10 pt-8 md:pt-10 pb-6 md:pb-8 md:pr-10">

                            {/* Top row */}
                            <div className="flex flex-col sm:flex-row items-start justify-between gap-8 sm:gap-0">
                                <div className="flex flex-col gap-4 md:gap-6">
                                    <h2 className="font-sato text-5xl md:text-7xl text-white/90 font-bold">PBeyond</h2>
                                    <div className="pt-2 md:pt-4">
                                        <p className="text-base md:text-lg font-sato text-white">Subscribe to our emails</p>
                                        <div className="flex items-center border-b border-white/40 pb-1 w-64 md:w-72">
                                            <input
                                                type="email"
                                                placeholder="Email"
                                ...
                                {/* Nav link groups */}
                                <div className="hidden flex gap-8 md:gap-12 md:mr-8">
                                    {links.map((link, i) => (
                                        <div key={i} className="space-y-3">
                                            <span className="block text-sm md:text-md font-semibold font-sato text-white">{link.group}</span>
                            ...
                            {/* Bottom row */}
                            <div className="flex flex-col items-end gap-3 mb-1 md:mb-2 md:mr-8">
