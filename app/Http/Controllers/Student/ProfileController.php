...
class ProfileController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            // Photo & Document uploads (PDF only for documents)
            'passport_photo'    => 'nullable|image|mimes:jpeg,png,jpg|max:2048',
            'ic_document'       => 'nullable|mimes:pdf|max:5120',
            'driver_license'    => 'nullable|mimes:pdf|max:5120',
            'academic_results'  => 'nullable|mimes:pdf|max:5120',
            ...
            'ic_document.mimes'      => 'Identity Card must be a PDF file.',
            'driver_license.mimes'   => 'Driver License must be a PDF file.',
            'academic_results.mimes' => 'Academic Results must be a PDF file.',
        ]);
        ...
        DB::transaction(function () use ($request, $student, $validated) {
            ...
            // Save directly to mobile_phone without calling phone_number
            ...
            if ($request->hasFile('passport_photo')) {
                ...
                $student->passport_photo_path = $request->file('passport_photo')->store('passport_photos', 'public');
            }

            // Documents
            if ($request->hasFile('ic_document')) {
                if ($student->ic_document) {
                    Storage::disk('public')->delete($student->ic_document);
                }
                $student->ic_document = $request->file('ic_document')->store('documents', 'public');
            }

            if ($request->hasFile('driver_license')) {
                if ($student->driver_license) {
                    Storage::disk('public')->delete($student->driver_license);
                }
                $student->driver_license = $request->file('driver_license')->store('documents', 'public');
            }

            if ($request->hasFile('academic_results')) {
                if ($student->academic_results) {
                    Storage::disk('public')->delete($student->academic_results);
                }
                $student->academic_results = $request->file('academic_results')->store('documents', 'public');
            }

            if (!empty($student->cv_generated_at)) {
                $student->cv_snapshot = null;
                $student->cv_generated_at = null;
            }
